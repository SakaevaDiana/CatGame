...
export default class QuestSystem {
  ...
  createPlaceholderDog() {
    ...
    const legPos = [[-0.18, 0.08, 0.2], [0.18, 0.08, 0.2], [-0.18, 0.08, -0.2], [0.18, 0.08, -0.2]];
  ...
  createBone() {
    ...
    leftKnob.position.set(-0.3, 0, 0);
    leftKnob.scale.set(1, 0.75, 0.75);
    group.add(leftKnob);

    const rightKnob = new THREE.Mesh(knobGeo, mat);
    rightKnob.position.set(0.3, 0, 0);
    rightKnob.scale.set(1, 0.75, 0.75);
    group.add(rightKnob);

    const shaft = new THREE.Mesh(
      new THREE.CylinderGeometry(0.05, 0.06, 0.45, 6),
      mat
    );
  ...
  updateQuestBubblePosition() {
    if (!this.questBubbleActive || !this.dogReady) return;
    const p = this.dog.position;
    const worldPos = new THREE.Vector3(p.x, p.y + 1.5, p.z);
  ...
  update(delta) {
    ...
    const distToBone = this.bone && this.bone.visible
      ? Math.sqrt((px - BONE_POS.x) ** 2 + (pz - BONE_POS.z) ** 2)
      : 999;
    ...
    if (this.questBubbleActive) {
      this.hideInteractPrompt();
      this.updateQuestBubblePosition();
      return;
    }
